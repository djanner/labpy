...
class Stanford830(Instrument):
    ...
    def __init__(self,name,address):
        ...
        self.status='Ready'
        self._sweeprate_auxout=0.0
        self._sweeprate_amplitude=0.0
        self._sweeprate_step=0.02
    ...
    def set_aux_sweeprate(self,rate):
        # rate is in V/s
        self._sweeprate_auxout=abs(rate)

    def get_aux_sweeprate(self):
        return self._sweeprate_auxout

    def set_amplitude_sweeprate(self,rate):
        # rate is in V/s
        self._sweeprate_amplitude=abs(rate)

    def get_amplitude_sweeprate(self):
        return self._sweeprate_amplitude

    def set_sweeprate_step(self,rate):
        # rate is in V/s
        self._sweeprate_step=abs(rate)

    def get_sweeprate_step(self):
        return self._sweeprate_step

    def __sweep_array(self,start,stop,rate):
        if (abs(start-stop)<1e-5) or (rate==0.0):
            sweep=[stop]
            dt_wait=0
        else:
            steps=round(abs(start-stop)/self._sweeprate_step)
            ds=abs(start-stop)/steps
            sweep = numpy.linspace(start,stop,steps)
            dt_wait=ds/rate #time to wait in seconds
        return sweep,dt_wait

    ############## Command: zero_all_out ##############
    '''
    Set all outputs to zero (all 4 AUX and Amplitude)
    '''

    def set_zero_all_out(self):
        self.set_amplitude(0.004)
        for i in range(1,5):
            self.set_aux_output(i)

    ############## Command: get_all_out ##############
    '''
    Get all outputs values (all 4 AUX and Amplitude)
    '''

    def get_all_out(self):
        ampl=self.get_amplitude()
        print('Ampl :\t%2.3f' % float(ampl))
        for i in range(1,5):
            aux=self.get_aux_output(i)
            print('Aux '+str(i)+':\t%2.3f' % float(aux))
    ...
    def set_amplitude (self,i):
        ...
        if i<0.004:
            i=0.004
        Vfinal=i
        Vactual=float(self.get_amplitude())
        V_sweep,dt_wait=self.__sweep_array(Vactual,Vfinal,self._sweeprate_amplitude)
        for V in V_sweep:
            commtxt="SLVL "
            commtxt=commtxt+str(i)
            self.write(commtxt)
    ...
    def set_aux_output (self,i,x=0.0):
        Vfinal=x
        Vactual=float(self.get_aux_output(i))
        V_sweep,dt_wait=self.__sweep_array(Vactual,Vfinal,self._sweeprate_auxout)
        for V in V_sweep:
            commtxt="AUXV "
            commtxt=commtxt+str(i)+","+str(V)
            self.write(commtxt)
            time.sleep(dt_wait)
